    ...
    return (
      <div className={styles.main}>
        <Card className="login-card" title="CMS 后台登录" extra={<Button type="danger">主页</Button>}>
          <LoginForm onTabChange={this.setType} activeKey={this.state.loginType} onSubmit={handleSubmit} className="login-form">
            <Tab key="account" tab="账户密码登录">
              {status === "error" && this.state.loginType === "account" && !submitting && (
            ...
            <Tab key="mobile" tab="手机号登录">
              {status === "error" && this.state.loginType === "mobile" && !submitting && (<LoginMessage content="验证码错误" />)}
            ...
            <div>
              <Checkbox checked={this.state.autoLogin} onChange={e => this.setAutoLogin(e.target.checked)}>自动登录</Checkbox>
